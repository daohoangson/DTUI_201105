...
*/
class XenForo_Template_Compiler_Tag_Admin_SelectUnit extends XenForo_Template_Compiler_Tag_Admin_Abstract implements XenForo_Template_Compiler_Tag_Interface
{
	/**
	...
	*/
	public function compile(XenForo_Template_Compiler $compiler, $tag, array $attributes, array $children, array $options)
	{
		...
		$controlOptions = $this->_getControlOptions($compiler, $attributes, array('size', 'multiple'));

		if (!isset($controlOptions['name']))
		{
			throw $compiler->getNewCompilerException(new XenForo_Phrase('select_tags_must_specify_name'));
		}
